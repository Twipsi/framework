...
class RedirectIfEmailNotVerified implements MiddlewareInterface
{
    /**
    ...
    public function resolve(Request $request, ...$args): ResponseInterface|bool
    {
        ...
        if(!$user || 
            $user instanceof IVerifiable
                && !$user->hasVerifiedEmail()) {

			return $this->sendUnverifiedResponse($request);
		}
	...
	/**
	 * Redirect user to account verification.
	 * 
	 * @return ResponseInterface
	 */
	protected function sendUnverifiedResponse(Request $request): ResponseInterface
	{
        return $request->isRequestAjax() 
            ? new JsonResponse(Translate::get('authentication.unverified'), 403)
		    : Redirect::to(Url::route('account.activation.index'));
	}
}
